...
const listProduct = ({ products }: ProductsProps) => {
  ...
  return (
    <div>
      ...
      <a href="admin/create">  <button className='bg-green-600 rounded-md'>Thêm mới</button></a>
                ...
                <tbody>
                  {data.map((item: any, index: any) => (
                    <tr key={index} className="border-solid	">
                      ...
                      <td className='border '>
                        <button className='bg-red-900 px-2 py-2 text-center rounded-md' onClick={() => remove(item.id)}>Delete</button>
                        <button className="bg-yellow-500 px-2 py-2 text-center rounded-md "> <Link href={`product/${item.id}`}>Edit</Link></button>
